...
def _run_numpy(
    ...
    for _ in range(niter):
        runner.rollout(
            model_list, data_list, initial_state, control,
            nstep=nstep, state=state_buf, sensordata=sensordata_buf,
        )
...
def _run_mlx(
    ...
    for _ in range(niter):
        out = runner.step(
            ...
            nstep=nstep, chunk_size=chunk_size, out_dtype=mx.float32,
        )
...
def _bench_one_task(
    ...
    for batch_size in batch_sizes:
        ...
        if _IS_MACOS:
            ...
            with mj_rollout.Rollout(nthread=actual_nthread) as numpy_runner, \
                ...
                _run_numpy(numpy_runner, model_list, data_list, initial_state, control,
                           state_buf, sensordata_buf, nstep, warmup)
                _run_mlx(mlx_runner, model_list, data_list, initial_state_mx, control_mx,
                         nstep, warmup, chunk_size)

                probe = mlx_runner.step(
                    model=model_list, data=data_list, initial_state=initial_state_mx,
                    control=control_mx, nstep=nstep, out_dtype=mx.float32,
                )
                output_shape_mode = _infer_output_shape_mode(probe)

                numpy_t = _run_numpy(numpy_runner, model_list, data_list, initial_state, control,
                                     state_buf, sensordata_buf, nstep, iters)
                mlx_t = _run_mlx(mlx_runner, model_list, data_list, initial_state_mx, control_mx,
                                  nstep, iters, chunk_size)

            records.append(BenchRecord(
                task=task_name, backend="numpy", batch_size=batch_size,
                nstep=nstep, nthread=actual_nthread, avg_time_sec=numpy_t,
                sps=batch_size * nstep / numpy_t, output_shape_mode="n/a",
            ))
            records.append(BenchRecord(
                task=task_name, backend="mlx_native", batch_size=batch_size,
                nstep=nstep, nthread=actual_nthread, avg_time_sec=mlx_t,
                sps=batch_size * nstep / mlx_t, output_shape_mode=output_shape_mode,
            ))
            print(
            ...
            with mj_rollout.Rollout(nthread=1) as runner1, \
                ...
                _run_numpy(runner1, model_list, data_list_1, initial_state, control,
                           state_buf, sensordata_buf, nstep, warmup)
                _run_numpy(runner_n, model_list, data_list_n, initial_state, control,
                           state_buf, sensordata_buf, nstep, warmup)
                t1 = _run_numpy(runner1, model_list, data_list_1, initial_state, control,
                                state_buf, sensordata_buf, nstep, iters)
                tn = _run_numpy(runner_n, model_list, data_list_n, initial_state, control,
                                state_buf, sensordata_buf, nstep, iters)

            records.append(BenchRecord(
                task=task_name, backend="rollout_1t", batch_size=batch_size,
                nstep=nstep, nthread=1, avg_time_sec=t1,
                sps=batch_size * nstep / t1, output_shape_mode="n/a",
            ))
            records.append(BenchRecord(
                task=task_name, backend=f"rollout_{nthread}t", batch_size=batch_size,
                nstep=nstep, nthread=nthread, avg_time_sec=tn,
                sps=batch_size * nstep / tn, output_shape_mode="n/a",
            ))
            print(
                f"[{task_name}] batch={batch_size:5d} "
...
def _plot_latency(records: List[BenchRecord], out_png: Path, batch_sizes: List[int], nthread: int):
    ...
    task_alpha = {"Go1JoystickFlatTerrain": 0.75, "Go2JoystickFlatTerrain": 0.9, "G1JoystickFlatTerrain": 1.0}
    task_hatch = {"Go1JoystickFlatTerrain": "//", "Go2JoystickFlatTerrain": "\\\\", "G1JoystickFlatTerrain": "xx"}
    ...
    for task_idx, task_name in enumerate(task_names):
        ...
        for b_idx, backend in enumerate(all_backends):
            ...
            ax.bar(x + offset, y, width=bar_width,
                   color=backend_colors[backend], alpha=task_alpha[task_name],
                   hatch=task_hatch[task_name], edgecolor="black", linewidth=0.2)
    ...
    backend_handles = [Patch(facecolor=backend_colors[b], edgecolor="black", label=backend_display[b]) for b in all_backends]
    task_handles = [
        Patch(facecolor="white", edgecolor="black", hatch=task_hatch["Go1JoystickFlatTerrain"], label="Task:Go1"),
        Patch(facecolor="white", edgecolor="black", hatch=task_hatch["Go2JoystickFlatTerrain"], label="Task:Go2"),
        Patch(facecolor="white", edgecolor="black", hatch=task_hatch["G1JoystickFlatTerrain"],  label="Task:G1"),
    ]
    fig.suptitle(
    ...
    fig.legend(
        handles=backend_handles + task_handles, fontsize=9,
        ncol=len(backend_handles + task_handles), loc="upper center",
        bbox_to_anchor=(0.5, 0.895), frameon=True, handlelength=1.8, columnspacing=1.2,
    )
...
def _plot_fps(records: List[BenchRecord], out_png: Path, batch_sizes: List[int]):
    ...
    for task_name in TASK_CONFIGS:
        for backend in sorted({r.backend for r in records if r.task == task_name}):
            recs = sorted([r for r in records if r.task == task_name and r.backend == backend],
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark parallel physics backends (rollout / mlx_step)")
    parser.add_argument("--nstep", type=int, default=1)
    parser.add_argument("--nthread", type=int, default=8)
    parser.add_argument("--warmup", type=int, default=5)
    parser.add_argument("--iters", type=int, default=10)
    parser.add_argument("--chunk-size", type=int, default=16, help="MLX step chunk size (macOS only)")
    ...
    parser.add_argument("--out-json", type=str,
                        default="benchmark/outputs/mlx_step/results.json")
    ...
    for task_name in task_names:
        records.extend(_bench_one_task(
            task_name=task_name, batch_sizes=batch_sizes,
            nstep=args.nstep, nthread=args.nthread,
            warmup=args.warmup, iters=args.iters, chunk_size=args.chunk_size,
        ))
        ...
        "summary": {
            "speedup_a_over_b": {
                task: {
                    ...
                    "backend_b": backend_b,
                    "geomean": _geomean([
                        next(r.avg_time_sec for r in records
                             if r.task == task and r.backend == backend_a and r.batch_size == b)
                        / max(
                            next(r.avg_time_sec for r in records
                                 if r.task == task and r.backend == backend_b and r.batch_size == b),
                            1e-12,
                        )
